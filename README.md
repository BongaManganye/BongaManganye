# Hi there, I'm Bonga Manganye 

## » About Me
I am a **Computer Science and Statistics** student and aspiring software developer based in Johannesburg, South Africa. My technical focus centers around bridging the gap between robust software engineering and data analytics building scalable data pipelines, writing clean automation scripts, and designing efficient relational database architectures.

---

## ¸ Tech Stack & Skills

* **Languages:** Python, SQL
* **Data Engineering & Analysis:** Pandas, NumPy, MySQL Workbench, Data Warehousing (Bronze, Silver, Gold Layers)
* **Web & Automation:** Streamlit, yt-dlp
* **Version Control & Tools:** Git, GitHub, Terminal

---

##  Projects & Portfolio

* **[Python-Mini-Projects](https://github.com/BongaManganye/Python-Mini-Projects):** A collection of functional Python scripts, utility tools, and coding challenge implementations.
* **Data Warehousing Architecture:** Designed and implemented a personal three-layer data warehouse utilizing Bronze, Silver, and Gold staging environments in MySQL for optimized data transformation.
* **TechSolutions Inc. Database System:** Engineered a centralized relational database management system to streamline multi-area business data processes.
* **Smart City Crime Prediction:** Developed and evaluated a predictive machine learning model analyzing urban data metrics for a smart city datathon.
* **TravelIntel AI:** Collaborated on the software design blueprint and architecture specifications for an AI-driven travel intelligence application.

---

##  GitHub Stats

> *"Data is the new oil, but software is the engine that runs it."*

Feel free to explore my repositories, check out my code, or reach out if you'd like to collaborate on data-driven projects!
